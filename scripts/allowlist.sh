...
init_allowlist() {
  ...
  # 🔒 Isoler dnsmasq sur l'interface AP pour éviter le conflit avec systemd-resolved
  WIFI_AP_CONF="/etc/dnsmasq.d/wifi-ap.conf"
  # Essaie de récupérer l'IP de l'AP (fallback 10.0.0.1 si non dispo)
...
EOF
  ...
  # Vérifier si le port 53 est pris par systemd-resolved pour info
  if sudo ss -ltnp '( sport = :53 )' 2>/dev/null | grep -q systemd-resolved; then
    LOGGER warn "Port 53 in use by systemd-resolved — dnsmasq is bound to $wireless_id:$AP_IP so it won't conflict."
  fi
