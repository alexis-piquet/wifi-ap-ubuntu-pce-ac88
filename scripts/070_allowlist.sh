...
_load_env() {
  ...
  # Ensure files exist, even empty ones (desired behavior)
  ...
  [[ -s "$WHITELIST_FILE" ]]     || LOGGER info "whitelist.txt is empty – no domain→ipset mapping will be created (that’s fine)."
...
_prepare_ipsets() {
  ...
  if [[ -s "$ALLOW_ALL_IPS_FILE" ]]; then
    # Lire lignes non vides / non commentées
...
_program_iptables_chain() {
  ...
  # Ensure WiFi→Eth traffic goes through the chain (header)
