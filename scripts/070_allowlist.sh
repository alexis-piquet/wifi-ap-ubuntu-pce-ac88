...
# ──────────────────────────────────────────────────────────────────────────────
# Globals (initialized to satisfy `set -u`; real values come from _load_env)
# ──────────────────────────────────────────────────────────────────────────────
NET_MODE=""; ETHERNET_IF=""; WIRELESS_IF=""
ALLOWLIST_DIR=""; WHITELIST_FILE=""; ALLOW_ALL_IPS_FILE=""
DNSMASQ_D=""; DNSMASQ_WHITELIST=""
IPSET_SERVICE_SRC=""; IPSET_SERVICE_DST=""

# Re-derive safe defaults inside any function that needs them (defensive).
_ensure_defaults() {
...
_load_env() {
  ...
  # Promote as globals + defaults
  declare -g NET_MODE="${NET_MODE:-router}"        # router | bridge
  declare -g ETHERNET_IF="${ETHERNET_IF:?Set ETHERNET_IF in .env}"
  declare -g WIRELESS_IF="${WIRELESS_IF:?Set WIRELESS_IF in .env}"

  declare -g ALLOWLIST_DIR="${ALLOWLIST_DIR:-$ROOT_DIR/config/allowlist}"
  declare -g WHITELIST_FILE="${WHITELIST_FILE:-$ALLOWLIST_DIR/whitelist.txt}"
  declare -g ALLOW_ALL_IPS_FILE="${ALLOW_ALL_IPS_FILE:-$ALLOWLIST_DIR/allow_all_ips.txt}"

  declare -g DNSMASQ_D="${DNSMASQ_D:-/etc/dnsmasq.d}"
  declare -g DNSMASQ_WHITELIST="${DNSMASQ_WHITELIST:-$DNSMASQ_D/whitelist.conf}"

  declare -g IPSET_SERVICE_SRC="${IPSET_SERVICE_SRC:-$ROOT_DIR/config/services/ipset-restore.service}"
  declare -g IPSET_SERVICE_DST="${IPSET_SERVICE_DST:-/etc/systemd/system/ipset-restore.service}"
  ...
  # Ensure allowlist dir + files exist (empty is OK)
  sudo install -d -m 0755 "$ALLOWLIST_DIR"
  ...
  [[ -s "$WHITELIST_FILE" ]]     || LOGGER info "whitelist.txt is empty – no domain→ipset mapping (that’s fine)."
  [[ -s "$ALLOW_ALL_IPS_FILE" ]] || LOGGER info "allow_all_ips.txt is empty – 'allow_all' set stays empty (that’s fine)."
}
...
_write_dnsmasq_whitelist() {
  ...
  if [[ -s "$WHITELIST_FILE" ]]; then
    # Write "ipset=/domain/whitelist" for each valid line
...
_program_iptables_chain() {
  ...
  # Remove potential permissive rule (060_nat)
  ...
  # Dedicated chain for readability
  sudo iptables -N WIFI_ALLOWLIST 2>/dev/null || true
  sudo iptables -F WIFI_ALLOWLIST

  # Policy: allow_all(src) → ACCEPT; whitelist(dst) → ACCEPT; else REJECT
  ...
  # Ensure WiFi→Eth traffic traverses the chain (insert at top)
