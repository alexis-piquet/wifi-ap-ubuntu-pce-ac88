...
# Remove iptables rules using ipsets (if they exist)
...
sudo ipset flush allow_all 2>/dev/null || true
sudo ipset destroy allow_all 2>/dev/null || true

sudo ipset flush whitelist 2>/dev/null || true
sudo ipset destroy whitelist 2>/dev/null || true

step "Creating ipsets"
sudo ipset create whitelist hash:ip 2>/dev/null || echo "whitelist ipset already exists"
sudo ipset create allow_all hash:ip 2>/dev/null || echo "allow_all ipset already exists"
